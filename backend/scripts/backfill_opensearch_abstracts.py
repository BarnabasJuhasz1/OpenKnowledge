...
This is the dev copy (plain-HTTP docker-compose OpenSearch). The production copy
deploy/opensearch-ingest/scripts/backfill_opensearch_abstracts.py differs only in the
OpenSearch client block — keep the rest identical.
...
Env (backend/.env): BIGQUERY_DATASET_REF, OPENSEARCH_URL, OPENSEARCH_INDEX.
"""
...
def main() -> None:
    ...
    # --- OpenSearch client (dev: plain HTTP, no auth) ---
